const ApiKey = "6b7c297ba0f14159991cccf38579bdc9";
    ...
    fetch(url, fetchHeader)
        ...
        .then(resJson => {
            ...
            resJson.squad.forEach(squads => {
                squad += `
                <li class="collection-item avatar">
                    <span class="title">${squads.name}</span>
                    <p>Posisi: ${squads.position} <br>
                       Kebangsaan: ${squads.nationality} <br> 
                       Negara Kelahiran : ${squads.countryOfBirth}
                    </p>
                    <a href="#" class="secondary-content"><i class="material-icons">info</i></a>
                </li>
                `
            });
            contents.innerHTML = '<ul class="collection">' + squad + '</ul>'
            // const detail = document.querySelectorAll('.secondary-content');
            // detail.forEach(btn=>{
            //     btn.onclick=(event)=>{
            //         showTeamInfo(event.target.dataset.id);
